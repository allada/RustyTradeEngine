...
fn main() {
  ...
  let thread_io = thread::Builder::new().name("IO".into()).spawn(move || threads::io::start(matcher_tx, io_rx)).unwrap();
  let thread_matcher = thread::Builder::new().name("Matcher".into()).spawn(move || threads::matcher::start(io_tx, matcher_rx)).unwrap();
  ...
  for i in 0..1_000_000 {
    ...
    thread_manager_matcher_tx.send(threads::MatcherThreadMessage::AddOrder(Order::new(i, i % 10, i % 10, side, OrderTypeT::LIMIT))).unwrap();
  }
  println!("Done Sending");

  thread_manager_matcher_tx.send(threads::MatcherThreadMessage::Exit).unwrap();
