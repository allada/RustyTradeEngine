...
impl DummyOrder {
  pub fn new(id: OrderIdT, price: PriceT, qty: QtyT, side: SideT, order_type: OrderTypeT) -> DummyOrder {
    DummyOrder {id: id, price: price, qty: qty, side: side, order_type: order_type}
  }
  ...
  pub fn verify(&self, order: &Order, ignore_qty: bool) {
    ...
    assert_eq!(*order.order_type(), self.order_type, "Order types do not match.");
    if !ignore_qty {
...
mod ledger {
  ...
  fn verify_trade(trade: &Trade, taker: &DummyOrder, maker: &DummyOrder, trade_price: PriceT, trade_qty: QtyT) {
    ...
    assert_eq!(*trade.price(), trade_price, "Trade price does not match the input param.");
    assert_eq!(*maker.price(), trade_price, "Input param price is expected to be maker price.");
    assert!(*taker.qty() == trade_qty || *maker.qty() == trade_qty, "Expected one of the orders to be of trade_qty {}", trade_qty);
  }
